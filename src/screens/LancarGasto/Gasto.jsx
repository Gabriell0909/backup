...
import { KeyboardAvoidingView, Platform, StatusBar, Text, View, ScrollView, TouchableOpacity, TextInput } from 'react-native';
         ...
         <ScrollView contentContainerStyle={{ flexGrow: 1, alignItems: 'center', gap: 30 }}>
            ...
            <View style={styles.group}>
               <View style={styles.containerViews}>
                  ...
                  <ButtonA
                     style={styles.button}
                     onPress={() => openBottomSheet('devedores')}
                  >
               ...
               <View style={styles.containerGroupButton}>
                  <ButtonA
                     style={styles.button}
                     onPress={() => openBottomSheet('contas')}
                  >
                  ...
                  <ButtonA
                     ...
                     onPress={() => openBottomSheet('categorias')}
                  >
                     <Ionicons name="layers-outline" size={32} />
                  </ButtonA>
                  <ButtonA
                     style={styles.calendario}
                     onPress={() => openBottomSheet('calendario')}
                  >
               ...
               <View>
                  ...
                  <View style={styles.containerGastos}>
                     <ButtonA
                        style={[styles.typeOfExpense, tipo === 'unico' && { borderWidth: 2, borderColor: '#0dd' }]}
                        onPress={() => {
                           setTipo('unico');
                     ...
                     <ButtonA
                        style={[styles.typeOfExpense, tipo === 'recorrente' && { borderWidth: 2, borderColor: '#0dd' }]}
                        onPress={() => {
                           setTipo('recorrente');
                     ...
                     <ButtonA
                        style={[styles.typeOfExpense, tipo === 'parcelado' && { borderWidth: 2, borderColor: '#0dd' }]}
                        onPress={() => {
                           setTipo('parcelado');
                           setShowRecorrente(false);
         ...
         <BottomCustom sheetRef={sheetRef}>
            {active === 'devedores' && (
               <View>
                  ...
                  {devedores.map((d) => (
                     <ButtonA
                        key={d.key}
                        style={{ marginBottom: 5, backgroundColor: devedor && devedor.key === d.key ? '#0dd' : '#fff' }}
                        onPress={() => {
                           setDevedor(d);
                           sheetRef.current?.close();
                        }}
                     >
                        <Text style={{ color: devedor && devedor.key === d.key ? '#fff' : '#000' }}>{d.nome}</Text>
                     </ButtonA>
                  ))}
               </View>
            )}
            ...
            {active === 'contas' && (
               <View>
                  ...
                  {contas.map((c) => (
                     <ButtonA
                        key={c.key}
                        style={{ marginBottom: 5, backgroundColor: conta && conta.key === c.key ? '#0dd' : '#fff' }}
                        onPress={() => {
                           setConta(c);
                           sheetRef.current?.close();
                        }}
                     >
                        <Text style={{ color: conta && conta.key === c.key ? '#fff' : '#000' }}>{c.nome}</Text>
                     </ButtonA>
                  ))}
               </View>
            )}
            ...
            {active === 'categorias' && (
               <View>
                  ...
                  {categorias.map((cat) => (
                     <ButtonA
                        key={cat.key}
                        style={{ marginBottom: 5, backgroundColor: categoria && categoria.key === cat.key ? '#0dd' : '#fff' }}
                        onPress={() => {
                           setCategoria(cat);
                           sheetRef.current?.close();
                        }}
                     >
                        <Text style={{ color: categoria && categoria.key === cat.key ? '#fff' : '#000' }}>{cat.nome}</Text>
                     </ButtonA>
                  ))}
               </View>
            )}
               ...
               <View style={{ width: '100%' }}>
                  <View>
                     <Calendar
                        ...
                        onDayPress={(d) => setData(d.dateString)}
            ...
            {active === 'recorrente' && tipo === 'recorrente' && (
               <View style={{ alignItems: 'center', gap: 10 }}>
                  ...
                  <ButtonA
                     style={[styles.typeOfExpenseButton, recorrencia === 'mensal' && { borderWidth: 2, borderColor: '#0dd' }]}
                     onPress={() => setRecorrencia('mensal')}
                  >
                     <Text style={{ color: '#000' }}>Mensal</Text>
                  </ButtonA>
                  <ButtonA
                     style={[styles.typeOfExpenseButton, recorrencia === 'anual' && { borderWidth: 2, borderColor: '#0dd' }]}
            ...
            {active === 'parcelas' && tipo === 'parcelado' && (
               <View style={{ alignItems: 'center', gap: 10 }}>
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 10, justifyContent: 'center' }}>
                     ...
                     >
                        <TextInput
                           style={{ fontSize: 18, minWidth: 40, textAlign: 'center', borderBottomWidth: 1, borderColor: '#0dd' }}
                           ...
                           keyboardType="numeric"
                           onBlur={() => {
                              if (parcelasInput !== '' && !isNaN(Number(parcelasInput)) && Number(parcelasInput) > 0) {
